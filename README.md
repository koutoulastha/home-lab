...
- **Chart versions are pinned:** every Helm source sets an explicit `targetRevision`, so `selfHeal` never upgrades a chart on its own. Upgrades are a deliberate commit. `cert-manager` and `traefik` used to float on `"*"` and were pinned to their running versions (`v1.21.1` / `41.4.0`) — note jetstack's chart versions carry a `v` prefix and Traefik's do not. To check what a multi-source app actually resolved, read the **plural** status fields (`.status.sync.revisions`, `.status.sourceTypes`); the singular ones are only populated for single-source apps and read `<none>` here.
